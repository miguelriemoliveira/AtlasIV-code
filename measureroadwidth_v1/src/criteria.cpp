...
int CR_IsCloseCross(void)
{
	...
	if (Time.Since.LastModeChange>4000 && SensorValues.Cross==1)
		return(1);
	else
		return(0);
...
// 		cout << "Ocupation.N[" << i << "].AreaOcupationPercent = " <<  Ocupation.N[i].AreaOcupationPercent << endl;
	...
	if (Time.Since.LastModeChange>4000 && macro_min(Ocupation.N[0].AreaOcupationPercent, Ocupation.N[0].AreaOcupationPercent)>40 /*&& Ocupation.N[2].AreaOcupationPercent < 20 */&& Mode.DistantCross==CROSSFOUND)
		return(1);
	else
		return(0);
