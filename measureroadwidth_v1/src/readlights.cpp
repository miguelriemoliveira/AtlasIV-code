...
{
	cvAnd( GI.Lights.S_Filter, GI.Lights.S_Filter, GI.Lights.SandV_Filter,NULL );
}
...
void RL_ShapeRecognition()
{
	...
	{	
		...
		/*cvShowImage("workimg",workimg);
		printf("width do MAR=%d\n",LightsAnalisys.MARectangle.width);
		printf("height do MAR=%d\n",LightsAnalisys.MARectangle.height);
		printf("Area do MAR=%d\n",LightsAnalisys.MARectangle.Area);
		printf("Area do hull=%d\n",LightsAnalisys.HullArea);
		...
		if (macro_isbetween((int)LightsAnalisys.ObjectOrientation ,OBJECTLEFTMINANGLE ,OBJECTLEFTMAXANGLE) && LightsAnalisys.OcupationPercent<MAXIMUMOCUPATIONPERCENT)
		{
			LightsAnalisys.DetectedShape=DS_OBJECTORIENTEDLEFT;
		}
		else if (macro_isbetween((int)LightsAnalisys.ObjectOrientation ,OBJECTUPMINANGLE ,OBJECTUPMAXANGLE) && LightsAnalisys.OcupationPercent<MAXIMUMOCUPATIONPERCENT)
		{
			LightsAnalisys.DetectedShape=DS_OBJECTORIENTEDUP;
		}
		else if ((macro_isbetween((int)LightsAnalisys.ObjectOrientation ,0 ,OBJECTRIGHTMINANGLE) || macro_isbetween((int)LightsAnalisys.ObjectOrientation ,OBJECTRIGHTMAXANGLE ,360)) && LightsAnalisys.OcupationPercent<MAXIMUMOCUPATIONPERCENT)
		{
		...
		else 
		{
			...
			printf("LightsAnalisys.MassCenter(x,y)=(%d,%d)\n",LightsAnalisys.MassCenter.x,LightsAnalisys.MassCenter.y);
...
float x_0,y_0,x_1,y_1;
	...
	x_0=(float)LightsAnalisys.MARectangle.Center[0];
	y_0=(float)LightsAnalisys.MARectangle.Center[1];
	
	x_1=LightsAnalisys.MassCenterFx;
	y_1=LightsAnalisys.MassCenterFy;
	
	m=((y_1-y_0)/(x_1-x_0));
	LightsAnalisys.ObjectOrientation=(180.0/M_PI)*(atan2((y_1-y_0),(x_1-x_0)));
	
	//printf("LightsAnalisys.ObjectOrientation(sem conices)=%g\n",LightsAnalisys.ObjectOrientation);
	if (LightsAnalisys.ObjectOrientation >0)
		LightsAnalisys.ObjectOrientation=LightsAnalisys.ObjectOrientation;
	else if (LightsAnalisys.ObjectOrientation <0)
		LightsAnalisys.ObjectOrientation=360+LightsAnalisys.ObjectOrientation;
