...
// 		printf("line=%d  || col=%d\n",line,col);
		...
		if (CrossAnalisys.PixelNum>MINIMUMCROSSSIZE/*to make sure its not just an isolated pix*/)
		{
			npts=0;
// TM_UpdateTick(&Time.Tick.Tick3);	
// 	unsigned int numberofpointtoextract = CurrentSpotsImg->height * CurrentSpotsImg->width;
// 	double min_val;
// 	double max_val;
// 	CvPoint min_loc;
// 	CvPoint max_loc;
// 	for(int countextract = 0; countextract < numberofpointtoextract; countextract++)
// 	{
// 		cvMinMaxLoc( CurrentSpotsMat, &min_val, &max_val, NULL, &max_loc );
// 		if (max_val == 255.0)
// 		{
// // 			pt0=cvPoint(col,line);
//  			npts++;
// 			cvSeqPush(CrossAnalisys.pts,&max_loc); 
// 			cvmSet(CurrentSpotsMat,max_loc.y, max_loc.x, 0.0);
// 		}
// 		else
// 		{
// 			break;
...
// 		cvWaitKey(0);
			if (CrossAnalisys.HeightWidthRatio<0.34 && (CrossAnalisys.MARAngle<10.0 && CrossAnalisys.MARAngle>-30.0))
			{
