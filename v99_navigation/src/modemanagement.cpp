...
static unsigned char i=0;
	...
	if (Mode.Mode==STOPMODE) 
	{
		...
		{
			#if USEPARKING
				if (RobotStatus.LapNumber>=TOTALLAPS)
				{
			...
			else if (LightsAnalisys.DetectedSymbol==DSy_YELLOWARROWLEFT)
	...
	else if(Mode.Mode==NORMALMODE)
	{
		...
		#if USELIGHTSREADING
			if (CR_IsCloseCross_1() && Time.Since.LastLapCount>3000 )
			{
			...
			else if(RobotStatus.PinsDetectedAll)
			{
	...
	else if(Mode.Mode==CONSTRUCTIONMODE)
	{
			...
			if (!RobotStatus.PinsDetectedDown)
			{			
				Mode.Mode=NORMALMODE;
...
static unsigned char i=0;
	...
	else if(Mode.Mode==NORMALMODE)
	{
			...
			if (CR_IsCloseCross() && Time.Since.LastLapCount>2000 )
			{
				...
				SendDirAndSpeed(RobotStatus.ChosenDA_PIC,STOPPEDSPEED,Handle.ComPort_0);
				Mode.Mode=STOPMODE;
				Mode.LastMode=NORMALMODE;
				RobotStatus.LapNumber++;
				TM_UpdateTick(&Time.Tick.LastLapCount);
