...
typedef struct{
		...
		int AdmissibleLowGoToPointWidth;
		...
		int AdmissibleTopGoToPointWidth;
		...
		int LowLeftGoToPoint_MoveClose[2];	//!< LowLeftGoToPoint line and column indexes of the pixel in question.
		char ValidLowLeftGoToPoint_MoveClose;	//!< ValidLowLeftGoToPoint validation flag.
		...
		int LowRightGoToPoint_MoveClose[2];	//!< LowRightGoToPoint line and column indexes of the pixel in question.
		char ValidLowRightGoToPoint_MoveClose;	//!< ValidLowRightGoToPoint validation flag.
		...
		int LeftHorizonSpace;
		unsigned char ObstacleIsLeft;
		int RightHorizonSpace;
		unsigned char ObstacleIsRight;
		unsigned char ObstacleIsInnerSide ;
...
typedef struct{
		...
		float LowRight_MoveClose[2];
		float LowLeft_MoveClose[2];
...
typedef struct{
		...
		TypeDriveAngleBy ConstDA_PP;			//!< DA_PP TypeDriveAngleBy struct using _PP analysis.
		...
		unsigned long PinsPixNumAll;
		unsigned long PinsPixNumDown;
		
		
		unsigned int TimesInEscapeMode;
		
		
		unsigned char OKRIGHT;
		unsigned char OKLEFT;
		}TypeRobotStatus;
...
typedef  struct{
		...
		int darkgreen[3];	//!< green color definition
		int blue[3];	//!< blue color definition
		int darkblue[3];	//!< blue color definition
		int yellow[3];	//!< yellow color definition
		int darkyellow[3];	//!< yellow color definition
		int cyan[3];	//!< cyan color definition
		int darkcyan[3];	//!< cyan color definition
		int orange[3];	//!< orange color definition
		int magenta[3];	//!< magenta color definition
		int darkmagenta[3];	//!< magenta color definition
...
typedef  struct{
		...
		int64 EscapeManouverTick1;
		int64 EscapeManouverTick2;
		int64 Tick1;
		int64 Tick2;
		int64 Tick3;
		int64 Tick4;
...
typedef struct{
		...
		float OrigMassCenterFx;			//!< Center of mass.
		float OrigMassCenterFy;			//!< Center of mass.
		...
		IplImage *GAU_Template;
 		CvMat *GAU_TemplateMat8U;
		CvMat *GAU_Results;
		double GAU_min_val,GAU_max_val;
		CvPoint GAU_min_loc,GAU_max_loc;
		
		IplImage *YAL_Template;
 		CvMat *YAL_TemplateMat8U;
		CvMat *YAL_Results;
		double YAL_min_val,YAL_max_val;
		CvPoint YAL_min_loc,YAL_max_loc;
		
		IplImage *YAR_Template;
 		CvMat *YAR_TemplateMat8U;
		CvMat *YAR_Results;
		double YAR_min_val,YAR_max_val;
		CvPoint YAR_min_loc,YAR_max_loc;
		
		IplImage *RC_Template;
 		CvMat *RC_TemplateMat8U;
		CvMat *RC_Results;
		double RC_min_val,RC_max_val;
		CvPoint RC_min_loc,RC_max_loc;
...
typedef struct{
		...
		unsigned char FindObstacle;
		
		IplImage *ParkImg;
		unsigned int PixCount;
		unsigned char ObstaclePosition;
...
typedef struct{
		...
		IplImage* RoadImg;
		IplImage* pTempLeftImage_ch1;
		IplImage* pTempLeftImage_ch2;
		IplImage* pTempLeftImage_ch3;
		}TypeOriginalImg;		
