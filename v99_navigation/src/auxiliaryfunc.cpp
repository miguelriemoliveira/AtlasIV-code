...
void AF_ClearBoxStruct(SquareTest *ST)
{
	...
	ST->ValidLowRightGoToPoint_MoveClose=0;
	ST->ValidLowLeftGoToPoint_MoveClose=0;
...
void AF_ClearRobotStatusStruct()
{
	...
	RobotStatus.DA_PP.LowRight_MoveClose[1]=0;
	RobotStatus.DA_PP.LowLeft_MoveClose[1]=0;
	
	RobotStatus.ConstDA_PP.BothLow[1]=0;
	RobotStatus.ConstDA_PP.BothTop[1]=0;
	RobotStatus.ConstDA_PP.LowLeft[1]=0;
	RobotStatus.ConstDA_PP.LowRight[1]=0;
	RobotStatus.ConstDA_PP.BothLeft[1]=0;
	RobotStatus.ConstDA_PP.BothRight[1]=0;
...
int i=99;
	...
	{
		#if USELIGHTSREADING
		if (Time.Since.LastStopMode<TIMETOSTARTLOOKINGFORCROSS && Mode.LastMode==STOPMODE)
		{
			{
		...
		{	
			#if USESPEEDCONTROL
				double average = (RobotStatus.LastDA[0] + RobotStatus.LastDA[1] + RobotStatus.LastDA[2] + RobotStatus.LastDA[3] + RobotStatus.LastDA[4])/5;
				
				if (average *1.2 > RobotStatus.ChosenDA && average *0.8 < RobotStatus.ChosenDA)
					RobotStatus.ChosenSpeed=NORMALSPEED+TURBOADDSPEED;
				else
					RobotStatus.ChosenSpeed=NORMALSPEED;
			#else
				RobotStatus.ChosenSpeed=NORMALSPEED;
		...
		else if (Mode.DistantCross==CROSSFOUND  && Time.Since.LastLapCount>TIMETOSTARTLOOKINGFORCROSS )
		{
			if (RobotStatus.LapNumber>=TOTALLAPS-1)
			{
// 				if (CrossAnalisys.MARMaxLineVal< 140)
// 					RobotStatus.ChosenSpeed = CROSSAPROACHSPEED+2;
// 				else if (CrossAnalisys.MARMaxLineVal< 180)
// 					RobotStatus.ChosenSpeed = CROSSAPROACHSPEED+1;
// 				else 
		...
		else if (Mode.DistantCross==NOCROSSFOUND && Time.Since.LastCross<=TIMETOWAITAFTERLASTCROSS && Time.Since.LastModeChange>TIMETOSTARTLOOKINGFORCROSS )
		{	
...
// 				else 
		...
		else if((SquareRegion.RelocatedLowGoToPoint || SquareRegion.RelocatedTopGoToPoint) && !macro_isbetween(RobotStatus.ChosenDA, 40, 50) )
		{
			RobotStatus.ChosenSpeed=DANGERSPEED;
...
{	
	...
	{	
		case TAKELEFT:
		{
			if (!RobotStatus.OKLEFT && RobotStatus.DA_PP.LowRight_MoveClose[1])
			{
				RobotStatus.ChosenDA=RobotStatus.DA_PP.LowRight_MoveClose[0];
				i=20;
				RobotStatus.ChosenGoToPoint[0]=SquareRegion.LowRightGoToPoint_MoveClose[0];
				RobotStatus.ChosenGoToPoint[1]=SquareRegion.LowRightGoToPoint_MoveClose[1];
			}
			else if (!RobotStatus.OKRIGHT && SquareRegion.ValidLowLeftGoToPoint_MoveClose)
			{
				RobotStatus.ChosenDA=RobotStatus.DA_PP.LowLeft_MoveClose[0];
				i=21;
				RobotStatus.ChosenGoToPoint[0]=SquareRegion.LowLeftGoToPoint_MoveClose[0];
				RobotStatus.ChosenGoToPoint[1]=SquareRegion.LowLeftGoToPoint_MoveClose[1];
			}
			else if (RobotStatus.DA_PP.BothTop[1])
			{
				RobotStatus.ChosenDA=RobotStatus.DA_PP.BothTop[0];
				i=1;
				RobotStatus.ChosenGoToPoint[0]=SquareRegion.TopGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=SquareRegion.TopGoToPoint[1];
			}
			else if(RobotStatus.DA_PP.BothLow[1])
			{
				RobotStatus.ChosenDA=RobotStatus.DA_PP.BothLow[0];
				i=3;
				RobotStatus.ChosenGoToPoint[0]=SquareRegion.LowGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=SquareRegion.LowGoToPoint[1];
			}
			else
			{
				if(RobotStatus.DA_PP.LowLeft[1])
				{
					...
					i=5;
					RobotStatus.ChosenGoToPoint[0]=SquareRegion.LowLeftGoToPoint[0];
					RobotStatus.ChosenGoToPoint[1]=SquareRegion.LowLeftGoToPoint[1];
				}
				else if(RobotStatus.DA_PP.LowRight[1])
				{	
					RobotStatus.ChosenDA=RobotStatus.DA_PP.LowRight[0];
					i=7;
					RobotStatus.ChosenGoToPoint[0]=SquareRegion.LowRightGoToPoint[0];
					RobotStatus.ChosenGoToPoint[1]=SquareRegion.LowRightGoToPoint[1];
				}
				else 
				{
			...
			if (SquareRegion.ObstacleIsInnerSide)
			{				
				RobotStatus.ChosenDA = RobotStatus.ChosenDA=RobotStatus.DA_PP.LowLeft[0];
		...
		case TAKERIGHT:
		{
			...
			else if (SquareRegion.RightLineAngle>140)
			{	
				if (RobotStatus.DA_PP.BothRight[1])
				{
					RobotStatus.ChosenDA=RobotStatus.DA_PP.BothRight[0];
					RobotStatus.ChosenGoToPoint[0]=SquareRegion.BothRightGoToPoint[0];
					RobotStatus.ChosenGoToPoint[1]=SquareRegion.BothRightGoToPoint[1];
					i=9;
				}
			}
			else
			...
			if (SquareRegion.ObstacleIsInnerSide)
			{				
				RobotStatus.ChosenDA = RobotStatus.ChosenDA=RobotStatus.DA_PP.LowRight[0];
...
{	
	switch (Mode.Direction)
	{
		case TAKELEFT:
		{
			if (RobotStatus.ConstDA_PP.BothTop[1])
			{
				RobotStatus.ChosenDA=RobotStatus.ConstDA_PP.BothTop[0];
				i=1;
				RobotStatus.ChosenGoToPoint[0]=ConstSquareRegion.TopGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=ConstSquareRegion.TopGoToPoint[1];
			}
			else if(RobotStatus.ConstDA_PP.BothLow[1])
			{
				RobotStatus.ChosenDA=RobotStatus.ConstDA_PP.BothLow[0];
				i=3;
				RobotStatus.ChosenGoToPoint[0]=ConstSquareRegion.LowGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=ConstSquareRegion.LowGoToPoint[1];
			}
			else if(RobotStatus.ConstDA_PP.LowLeft[1])
			{
				RobotStatus.ChosenDA=RobotStatus.ConstDA_PP.LowLeft[0];
				i=5;
				RobotStatus.ChosenGoToPoint[0]=ConstSquareRegion.LowLeftGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=ConstSquareRegion.LowLeftGoToPoint[1];
			}
			else if(RobotStatus.ConstDA_PP.LowRight[1])
			{	
				RobotStatus.ChosenDA=RobotStatus.ConstDA_PP.LowRight[0];
				i=7;
				RobotStatus.ChosenGoToPoint[0]=ConstSquareRegion.LowRightGoToPoint[0];
				RobotStatus.ChosenGoToPoint[1]=ConstSquareRegion.LowRightGoToPoint[1];
			}
			else 
			{
				RobotStatus.ChosenDA=RobotStatus.LastDA[0];	
...
// 	cvCvtPixToPlane(LightsImg_Original, GI.Orig.pTempLeftImage_ch1,NULL,NULL,NULL); 
// 	cvCvtPixToPlane(LightsImg_Original, NULL, GI.Orig.pTempLeftImage_ch2,NULL,NULL); 
// 	cvCvtPixToPlane(LightsImg_Original, NULL, NULL,GI.Orig.pTempLeftImage_ch3,NULL); 
// 	
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch1 , GI.Orig.pTempLeftImage_ch1 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch2 , GI.Orig.pTempLeftImage_ch2 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch3 , GI.Orig.pTempLeftImage_ch3 );
// 	
// 	cvMerge(GI.Orig.pTempLeftImage_ch1, GI.Orig.pTempLeftImage_ch2, GI.Orig.pTempLeftImage_ch3,NULL,LightsImg_Original);	
// 	
	
	
	//cvRectangle(LightsImg_Original,cvPoint(0,140),cvPoint(LightsImg_Original->width,LightsImg_Original->height),CV_RGB(0,0,0),-1);
	cvSetImageROI(LightsImg_Original,LIGHTSIMAGEROI);
	cvSetImageROI(GI.Lights.HSV,LIGHTSIMAGEROI);
	cvCvtColor(LightsImg_Original,GI.Lights.HSV,CV_RGB2HSV);	
	cvResetImageROI(LightsImg_Original);
	cvResetImageROI(GI.Lights.HSV);
...
// 	cvCvtPixToPlane(GI.Orig.LeftCam, GI.Orig.pTempLeftImage_ch1,GI.Orig.pTempLeftImage_ch2,GI.Orig.pTempLeftImage_ch3,NULL); 
// 
// 	
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch1 , GI.Orig.pTempLeftImage_ch1 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch2 , GI.Orig.pTempLeftImage_ch2 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch3 , GI.Orig.pTempLeftImage_ch3 );
// 	
// 	cvMerge(GI.Orig.pTempLeftImage_ch1, GI.Orig.pTempLeftImage_ch2, GI.Orig.pTempLeftImage_ch3,NULL,GI.Orig.LeftCam);
...
// 	cvCvtPixToPlane(GI.Orig.RightCam, GI.Orig.pTempLeftImage_ch1,NULL,NULL,NULL); 
// 	cvCvtPixToPlane(GI.Orig.RightCam, NULL, GI.Orig.pTempLeftImage_ch2,NULL,NULL); 
// 	cvCvtPixToPlane(GI.Orig.RightCam, NULL, NULL,GI.Orig.pTempLeftImage_ch3,NULL); 
// 	
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch1 , GI.Orig.pTempLeftImage_ch1 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch2 , GI.Orig.pTempLeftImage_ch2 );
// 	cvEqualizeHist( GI.Orig.pTempLeftImage_ch3 , GI.Orig.pTempLeftImage_ch3 );
// 	
// 	cvMerge(GI.Orig.pTempLeftImage_ch1, GI.Orig.pTempLeftImage_ch2, GI.Orig.pTempLeftImage_ch3,NULL,GI.Orig.RightCam);
// 	
	...
	if(! joinImages( GI.Orig.LeftCam_Undistorted , GI.Orig.RightCam_Undistorted , GI.Orig.RoadImg, CamCalibparams.overlapH, 0/*CamCalibparams.bin*/, CamCalibparams.binOper ))
    	{
	    printf("Could not join images. Aborting\n");
	    exit(0); 
    	}
	
	cvCvtPixToPlane(GI.Orig.RoadImg,NULL,NULL,RoadImg_gray,0);		
	cvThreshold( RoadImg_gray, RoadImg_gray, CamCalibparams.bin & 0xFF, 255, CV_THRESH_BINARY );

	
// 	cvNamedWindow("LEFT",1);
//  	cvShowImage("LEFT",GI.Orig.LeftCam);
// 	cvNamedWindow("RIGHT",1);
//  	cvShowImage("RIGHT",GI.Orig.RightCam);
//  	cvNamedWindow("JOIN",1);
// 	cvShowImage("JOIN",GI.Orig.RoadImg);
// 	cvWaitKey(0);	
	
	
	CleanIsolatedPoints(RoadImg_gray, RoadImg_gray, 1);  //1 to eliminate all types of isolated
...
{
unsigned int DirtoSend=45;


	TM_UpdateTick(&Time.Tick.EscapeManouverTick1);
	
	if (RobotStatus.ChosenDA_PIC>45)
		DirtoSend = 0;
	else
		DirtoSend = 90;
		
	SendDirAndSpeed(DirtoSend,BACKWARDSPEED,Handle.ComPort_0);
	
	
TM_UpdateTick(&Time.Tick.EscapeManouverTick2);
while ((Time.Tick.EscapeManouverTick2-Time.Tick.EscapeManouverTick1)/(Time.Tickspermicrosec*1000)<TIMETOHOLDINESCAPEMANOUVER)
	TM_UpdateTick(&Time.Tick.EscapeManouverTick2);
	
//RobotStatus.ChosenSpeed = 2;
RobotStatus.TimesInEscapeMode++;
	
#if IPAINT
	char str[255];
	sprintf(str,"DANGER (%d)",RobotStatus.TimesInEscapeMode);
	cvPutText(RoadImg_rgb_hough,str, cvPoint(200,200),&hugefont, CV_RGB(255,0,0));		
#endif 	
...
void AF_ClearAllStruct(void)
{
	...
	#if USECONSTRUCTIONMODE
	AF_ClearBoxStruct(&ConstSquareRegion);
	#endif
